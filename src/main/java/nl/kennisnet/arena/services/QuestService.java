...
public class QuestService extends HibernateAwareService {
   ...
   public void getQuest(final Long questId, final TransactionalCallback<Quest> callback) {
   ...
   public void buildQrCodesPdf(String url, long questId, OutputStream outputStream) {
      QrCodesPdfBuilder builder = new QrCodesPdfBuilder(url, participantService.getAllParticipants());
   ...
   public long participateInQuest(long participantId, Quest quest) {
      ...
      if (participations.size() > 0) {
         ...
         Participation participation = new Participation(participant, quest, INITIAL_SCORE);
         return merge(participation).getId();
      }
   }

   public QuestDTO save(QuestDTO questDTO) {
      Quest quest = DomainObjectFactory.create(questDTO);
      if (quest.getId() != null) {
         Quest originalQuest = get(Quest.class, quest.getId());
   ...
   private boolean savedByOwner(Quest quest, Quest originalQuest) {
      return originalQuest != null && originalQuest.getEmailOwner().equals(quest.getEmailOwner());
   }
   ...
   private void sendNotification(Quest quest) {
      try {
         ...
         buildQrCodesPdf(createGamarayUrl(quest), quest.getId(), outputStream);
         byte[] byteArray = outputStream.toByteArray();
         InputStreamSource source = new ByteArrayResource(byteArray);
         helper.addAttachment(String.format("ToegangsTickets_%s_%s.pdf",quest.getName(),currentShortDate(new Locale("nl"))), source);
   ...
   private String createGamarayUrl(Quest quest) {
      ...
      String url = String.format("%sgamaray/%d.gddf", baseUrl, quest.getId());
      return url;
   }
   ...
   private String createMailText(Quest quest) {
      ...
      String baseUrl = UtilityHelper.getBaseUrl(configuration);
      String designerUrl = String.format("%smain.do?arenaId=%s", baseUrl, quest.getId());
      // String monitorUrl = String.format("%s/ARenaMonitor.html?arenaId=%s",
      // baseUrl, quest.getId());

      result.append("Beste " + quest.getEmailOwner() +newLine+newLine);
      result.append("U heeft op " + currentDate(new Locale("nl")) + " om " + currentTime(new Locale("nl"))
            + " uw ontwerp opgeslagen:"+newLine);
      result.append("Arena : " + quest.getName() + " "+newLine);

      result.append("Opgeslagen op: " + designerUrl + ""+newLine);
      result.append(""+newLine);
      result.append("U kunt uw ontwerp op ieder moment aanpassen via de bovenstaande link. "+newLine);
      result.append(""+newLine);
      result.append("Gebruik maken van de Arena"+newLine);

      result.append("ARena " + quest.getName() + " is nu actief en deelnemers kunnen hem 24 uur/dag ervaren. "+newLine);

      result.append("Om de deelnemers de Arena te laten ervaren dienen zij in het bezit te zijn van een toegangsticket. "
            + "Door de toegangstickets (PDF) uit te printen en aan de deelnemers verstrekken hebben zij toegang tot de Arena."+newLine);
      result.append(""+newLine);
      ...
      result.append("Via onderstaande link vindt u direct de resultaten van de deelnemende teams." +
      		" Het overzicht wordt Live ververst, bij iedere activiteit in uw ARena."+newLine);
      result.append(newLine);
      
      result.append("Het PDF bestand kan niet worden geopend"+newLine);
      result
            .append("Het PDF bestand bevat de elektronische toegangstickets. Om dit bestand te openen heeft u de software “Adobe Reader” nodig. "
                  + "Installeer de gratis software Adobe Reader (http://get.adobe.com/reader/)  en open het bestand nogmaals "+newLine);
      result.append(""+newLine);
      result.append(""+newLine);
      ...
      result.append("ARena is een experimenteel project. Wij ontvangen graag uw verbetersuggesties, ervaringen en vragen."+newLine);
